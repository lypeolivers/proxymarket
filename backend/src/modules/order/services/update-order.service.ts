...
import { assertStatusTransition, resolveItemArtStatus, resolveOrderHeader } from './order-helpers';
...
export class UpdateOrderService {
  async execute(id: number, data: TUpdateOrderBody): Promise<TUpdateOrderResponse> {
    ...
    const result = await runInTransaction(async (transaction) => {
      ...
      await replaceOrderItems(id, header.order_status, data.items, transaction);
