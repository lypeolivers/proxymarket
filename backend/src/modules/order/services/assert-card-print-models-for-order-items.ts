...
export async function assertCardPrintModelsBelongToCards(
  items: Array<{ card_id: number; card_print_model_id: number | null }>,
  transaction: PrismaTransactionalClient
): Promise<void> {
  const withModel = items.filter((item) => item.card_print_model_id != null);
  if (withModel.length === 0) {
    return;
  }

  const ids = [...new Set(withModel.map((item) => item.card_print_model_id!))];
