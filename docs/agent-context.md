...
| **Modelo de impressão (`CardPrintModel`)** | Por **`Card`**: nome do modelo + **`file_name`** (rótulo para a gráfica). CRUD em **`/card-print-model`**; UI **`/modelos-carta`**. | Migração criou modelo **Legado** por carta usada em itens antigos. Verniz por linha de pedido (**`has_varnish`**, D32), não por duplicar modelo. |
...
### 2026-07-24 — D32: Verniz por linha de pedido (`has_varnish`)

- **Contexto:** o fluxo antigo duplicava **`CardPrintModel`** (base + sufixo `-VERNIZ`) só para controlar arquivos distintos na gráfica; o arquivo final agora é montado manualmente e o verniz é decisão por unidade na produção.
- **Decisão:** coluna **`order_item.has_varnish`** (default `false`); checkbox **Com verniz** no formulário de Pedidos; resumo de remessa/clipboard agrupa por modelo **e** verniz. Script manual **`backend/prisma/scripts/merge-varnish-print-models.sql`** unifica pares base/verniz no catálogo (soft delete dos duplicados, reponta FKs, backfill `has_varnish`); modelos verniz **órfãos** ficam para revisão manual.
...
- ✅ Verniz por linha (`has_varnish`): checkbox em Pedidos, badge em Produção, script SQL de merge de modelos duplicados (D32).
